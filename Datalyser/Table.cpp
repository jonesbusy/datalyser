...
{
    // Test de coh�rence de la taille
    if (this->size < 0)
...
{
    // D�sallocation et r�initialisation des lignes
    for (register long i = 0; i < this->size; i++)

        if (this->rows[i] != NULL)
        {
            // D�sallocation de la ligne
            delete this->rows[i];

            // R�initialisation du pointeur (ligne vide)
            this->rows[i] = NULL;
        }
}
...
void Table<Key, Container>::reverseRows(long index1, long index2)
{
    ...
    // Pointeur sur la premi�re ligne
    Container* rowTemp = this->rows[index1];

    // Remplacement de la premi�re par la seconde
    this->rows[index1] = this->rows[index2];

    // Placement de la premi�re dans la seconde
